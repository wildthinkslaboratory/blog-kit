...
function computeC() {
  ...
  let rootF = function (x) {
    return df(x) - secantSlope;
  };
...
let point2 = workspace.board.create(
  "point",
  [
    function () {
      return c + 1;
...
let cpoint = workspace.board.create("point", [cGet, fcGet], {
  name: "",
  color: cs.lightAnnote,
  fixed: true,
  visible: false,
});
...
let avert = workspace.board.create(
  "segment",
  [
    [aGet, 0],
    [aGet, faGet],
...
let bvert = workspace.board.create(
  "segment",
  [
    [bGet, 0],
    [bGet, fbGet],
...
let cvert = workspace.board.create(
  "segment",
  [
    [cGet, 0],
    [cGet, fcGet],
...
let rect1 = workspace.board.create(
  "polygon",
  [
    [aGet, 0],
    [aGet, dfcGet],
...
let secantRise = workspace.board.create(
  "segment",
  [
    [bGet, faGet],
    [bGet, fbGet],
...
let slope = workspace.board.create(
  "segment",
  [
    [aGet, faGet],
    [bGet, fbGet],
...
let height = workspace.board.create(
  "segment",
  [
    [
      function () {
        return xint.X2() + xint.Xerror;
      },
      0,
    ],
    [
...
formula3.onmouseover = onAFFactory(
  formula3,
  showAFFactory([rect1, secantRise]),
);
formula3.onmouseout = offAFFactory(
  formula3,
  hideAFFactory([rect1, secantRise]),
);
