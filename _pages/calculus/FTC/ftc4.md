...
We can see that adding up the rises of the secants is the same as the [change](:=showTotal=true) in $F$ between $a$ and $b$. This gives us the area under the curve.
...
let integral = workspace.board.create(
  "integral",
  [[a, b], workspace.functions[DF_id].graph],
  {
    visible: true,
...
let slider = new IntSlider(
  xintSR.board,
  [xintSR.attachRightX, xintSR.attachY],
  [1, 50],
  "N",
);
slider.setValue(n);
...
let bText = workspace.board.create(
  "text",
  [
    bGet,
    function () {
...
let totalRise = workspace.board.create(
  "segment",
  [
    [
      function () {
...
let riseText = workspace.board.create(
  "text",
  [
    function () {
      return xintSR.X2() + 0.2;
    },
    function () {
      return f(xintSR.X1()) + (f(xintSR.X2()) - f(xintSR.X1())) / 2;
    },
    "F(b) - F(a)",
  ],
