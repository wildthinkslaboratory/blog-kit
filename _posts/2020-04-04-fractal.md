...
function generateColorSeed() {
  ...
  base[0] = Math.floor(Math.random() * 9) * 64;
  base[1] = Math.floor(Math.random() * 9) * 64;
  base[2] = Math.floor(Math.random() * 9) * 64;
  ...
  for (let i=0; i < 3; i++) {
    ...
    for (let j=0; j < 6; j++) {
      ...
      if ((inc == 0 || up) && inc != 8) {
        color_incs[i][j] = Math.floor(Math.random() * (9 - inc));
      } 
      else {
